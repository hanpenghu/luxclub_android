...
public class RecyclerViewPager extends RecyclerView {
    ...
    // 控制页面切换速度，xml中不可配, 默认为 1
    private int mFlingTime = 5;
    ...
    boolean isInertia = false; // inertia slide state
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyle) {
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RecyclerViewPager, defStyle,
                0);
        mFlingFactor = a.getFloat(R.styleable.RecyclerViewPager_rvp_flingFactor, mFlingFactor);
        mTriggerOffset = a.getFloat(R.styleable.RecyclerViewPager_rvp_triggerOffset, mTriggerOffset);
        mSinglePageFling = a.getBoolean(R.styleable.RecyclerViewPager_rvp_singlePageFling, mSinglePageFling);
        isInertia = a.getBoolean(R.styleable.RecyclerViewPager_rvp_inertia, isInertia);
        mMillisecondsPerInch = a.getFloat(R.styleable.RecyclerViewPager_rvp_millisecondsPerInch, mMillisecondsPerInch);
        a.recycle();
    }
            ...
            LinearSmoothScroller linearSmoothScroller =
                    new LinearSmoothScroller(getContext()) {
                        ...
                        protected void onTargetFound(View targetView, RecyclerView.State state, Action action) {
                            ...
                            final int time = calculateTimeForDeceleration(distance * mFlingTime);
                            if (time > 0) {
