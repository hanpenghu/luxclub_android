...
import com.hsbank.luxclub.adapter.MyRecordsHistoryAdapter;
 ...
 */
public class RecordsHistoryActivity extends ProjectBaseActivity {
    /**页容量*/
    ...
    /**我的消费记录列表适配器*/
    private MyRecordsHistoryAdapter myRecordsConsumptionAdapter = null;
    /**分页加载是否还有更多数据*/
    ...
    protected int getLayoutId() {
        return R.layout.activity_my_records_history;
    }
    ...
    public void onEventMainThread(BaseProjectEvent event) {
        ...
        if(event.getCommand().equals(MyEvent.COMMAND_MY_RECORDS_CONSUMPTION)){
    ...
    protected void viewHandler() {
        ...
        myRecordsConsumptionAdapter = new MyRecordsHistoryAdapter(this);
    ...
    private void getData() {
        if(!TextUtils.isEmpty(SharedPreferencesUtil.getInstance().getToken())){
            ProviderFactory.getInstance().luxclub_feeRecordPageList(mContext,SharedPreferencesUtil.getInstance().getToken(), ProjectUtil.getCardNo(),
                    "0",PAGE_SIZE, pageNumber,feeRecordCallback);
        }
    }
    ...
    private AdapterView.OnItemClickListener listItemClickListener = new AdapterView.OnItemClickListener() {
        public void onItemClick(AdapterView<?> adapterView, View view,
            ...
            final Map<String, Object> dataItem = myRecordsConsumptionAdapter.datas.get(positionId);
            final String orderId = MapUtil.getString(dataItem, "orderId");
            final ProgressDialog dialog = new ProgressDialog(mContext);
            dialog.show();
            ProviderFactory.getInstance().luxclub_orderDetails(mContext, SharedPreferencesUtil.getInstance().getToken(), orderId, new ApiCallback() {
                @Override
                public void onResponse(ApiResponseBean response) {
                    if (response.getCode() == ApiCodeConstant.SUCCESS_CODE) {
                        ...
                        Map<String, Object> map = JsonUtil.toMap(response.getData());
                        OrderDetailActivity.show(mContext, orderId, MapUtil.getInt(map, "orderState"));
                    } else {
                        AlertUtil.getInstance().alertMessage(RecordsHistoryActivity.this, response.getText());
                    }
                }
            });
    ...
    private ApiCallback feeRecordCallback = new ApiCallback() {
        ...
        public void onResponse(ApiResponseBean response) {
            if (response.getCode() == ApiCodeConstant.SUCCESS_CODE) {
                LogUtil.i2(response.getData());
                EventBus.getDefault().post(new MyEvent(MyEvent.COMMAND_MY_RECORDS_CONSUMPTION, response.getData()));
            } else {
                AlertUtil.getInstance().alertMessage(RecordsHistoryActivity.this, response.getText());
            }
        }
    ...
    public static void show(Context context) {
        ...
        intent.setClass(context, RecordsHistoryActivity.class);
