...
import com.hsbank.luxclub.adapter.MyCoinRechargeHistoryAdapter;
 ...
 */
public class CoinPurseRechargeHistory extends ProjectBaseActivity {
    /**页容量*/
    ...
    /**我的充值记录列表适配器*/
    private MyCoinRechargeHistoryAdapter myCoinRechargeHistoryAdapter = null;
    /**分页加载是否还有更多数据*/
    ...
    protected int getLayoutId() {
        return R.layout.activity_coin_purse_recharge_history;
    }
    ...
    public void onEventMainThread(BaseProjectEvent event) {
        ...
        if(event.getCommand().equals(MyEvent.COMMAND_MY_RECHARGE_HISTORY)){
    ...
    private void computeData(List<Map<String, Object>> data) {
        ...
        myCoinRechargeHistoryAdapter.setList(list);
    ...
    private void updateUi() {
        ...
        boolean isListNoData = myCoinRechargeHistoryAdapter.getCount() == 0;
        if (data != null && data.size() > 0) {
            hasMore = data.get(data.size() - 1) != null;
            pullToRefreshView.onHeaderRefreshComplete();
            pullToRefreshView.onFooterRefreshComplete();
            myCoinRechargeHistoryAdapter.addDatas(data);
            ...
            if (isListNoData) {
                ...
                mViewHelper.setText(R.id.txt_no_data, getString(R.string.txt_no_data3));
            } else {
                pullToRefreshView.setVisibility(View.VISIBLE);
            }
        }
    ...
    protected void viewHandler() {
        setToolbarTitle(R.string.txt_recharge);
        ...
        myCoinRechargeHistoryAdapter = new MyCoinRechargeHistoryAdapter(this);
        listView.setAdapter(myCoinRechargeHistoryAdapter);
//        listView.setOnItemClickListener(itemClickListener);
    ...
    private OnHeaderRefreshListener onHeaderRefreshListener = new OnHeaderRefreshListener() {
        ...
        public void onHeaderRefresh(PullToRefreshView view) {
            pageNumber = 1;
            myCoinRechargeHistoryAdapter.clearDatas();
            dataList.clear();
            ProviderFactory.getInstance().walletfee_walletFeePageList(mContext,SharedPreferencesUtil.getInstance().getToken(), ProjectUtil.getCardNo(),
                    "1", PAGE_SIZE, pageNumber, feeRecordCallback);
        }
    ...
    private AdapterView.OnItemClickListener listItemClickListener = new AdapterView.OnItemClickListener() {
        public void onItemClick(AdapterView<?> adapterView, View view,
            ...
            final String feeId = MapUtil.getString(myCoinRechargeHistoryAdapter.datas.get(positionId), "feeId");
            RecordsHistoryDetailActivity.show(CoinPurseRechargeHistory.this);
    ...
    private ApiCallback feeRecordCallback = new ApiCallback() {
        ...
        public void onResponse(ApiResponseBean response) {
            if (response.getCode() == ApiCodeConstant.SUCCESS_CODE) {
                EventBus.getDefault().post(new MyEvent(MyEvent.COMMAND_MY_RECHARGE_HISTORY, response.getData()));
            } else {
                AlertUtil.getInstance().alertMessage(CoinPurseRechargeHistory.this, response.getText());
            }
        }
    };
    ...
    public static void show(Context context) {
        ...
        intent.setClass(context, CoinPurseRechargeHistory.class);
