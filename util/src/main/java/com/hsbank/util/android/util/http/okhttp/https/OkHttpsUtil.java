...
public class OkHttpsUtil {
     ...
     * @param okHttpClient
     */
    public static void setDefaultCertificates(OkHttpClient okHttpClient) {
        try {
            InputStream is = new Buffer().writeUtf8(OkHttpsConstant.HTTPS_KEY_CONTENT).inputStream();
            TrustManager[] trustManagers = prepareTrustManager(is);
            KeyManager[] keyManagers = prepareKeyManager(is, OkHttpsConstant.HTTPS_KEY_PASSWORD);
    ...
    /**
     * 设置证书
     * 例：OkHttpsUtil.setCertificates(context.getAssets().open("fakeapi.fdjf.net.cer"));
     * @param okHttpClient
     * @param certificates
     */
    public void setCertificates(OkHttpClient okHttpClient, InputStream... certificates) {
        try {
            ...
            sslContext.init(null,trustManagerFactory.getTrustManagers(), SecureRandom.getInstance("SHA1PRNG", "Crypto"));
            okHttpClient.setSslSocketFactory(sslContext.getSocketFactory());
        } catch (Exception e) {
            e.printStackTrace();
        }
