package com.hsbank.util.java.tool.loan.credit_assignment;
...
public class CreditAssignment implements ICreditAssignment {
	...
	@Override
	public Map<String, List<RepaymentPlanItem>> generate(double totalLoan, double assignmentLoan, double remainingLoan, Date assignmentDate, List<RepaymentPlanItem> repaymentPlan) {
			...
			for (RepaymentPlanItem item : repaymentPlan) {
				...
				if (assignmentDate.getTime() >=beginDate.getTime() && assignmentDate.getTime() <= endDate.getTime()) {
					...
					if (interest > 0) {
						...
						assignmentItem.setInterest(LoanUtil.formatAmount(interestRemaining * (assignmentLoan / totalLoan)));
					...
					double interest = item.getInterest();
					if (interest > 0) {
						assignmentItem.setInterest(LoanUtil.formatAmount(interest * (assignmentLoan / totalLoan)));
